...
for i=1:1000
    ...
    h=optimset('MaxFunEvals',20000,...
        ...
        'TolFun',1e-10);
    % Now run the fitting
    [parameter_hat,RESNORM,EXITFLAG,OUTPUT, gradient,hessian]=fminunc('BallStickSSD',startx,h,Avox,bvals,qhat);
...
% Now we calculate all 2 sigma ranges
x = TrueParameters;
h = 2 * sqrt(diag(TrueHessian));

S0conf = [x(1) - h(1) , x(1) + h(1)];
dconf = [x(2) - h(2), x(2) + h(2)];
fconf = [x(3) - h(3), x(3) + h(3)];
