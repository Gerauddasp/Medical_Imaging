...
BestStart = [1.0051, 1.1825e-09, 0.9093, -17.2758, 0.0393, 33.7591, 0.0367];
...
for T=1:10
    ...
    for i=1:2000
        ...
        startx(6) = BestStart(5) + BestStart(5) * r(3);
        startx(7) = BestStart(6) + BestStart(6) * r(4);
        ...
        h=optimset('MaxFunEvals',20000,...
            ...
            'TolFun',1e-10);
        % Now run the fitting
        [parameter_hat,RESNORM,EXITFLAG,OUTPUT]=fminunc('ZeppelinAndStick',startx,h,meas,bvals,grad_dirs);
        parameter_hat(1) = abs(parameter_hat(1));
        parameter_hat(2) = abs(parameter_hat(2));
        parameter_hat(3) = sin(parameter_hat(3))^2;
        if parameter_hat(6) < parameter_hat(7)
            temp = parameter_hat(6);
            parameter_hat(6) = parameter_hat(7);
            parameter_hat(7) = temp;
            clear('temp');
        end
...
plot(Avox, ' bs', 'MarkerSize', 2, 'LineWidth', 1);
hold on;
% Add the predictions to the plot!
plot(S, ' rx', 'MarkerSize', 2, 'LineWidth', 1);
